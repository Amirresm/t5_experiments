...
def main():
    ...
    def preprocess_decoder_only_function(examples):
        ...
        for i in range(len(examples[text_column])):
            if examples[text_column][i] and (summary_column == "NONE" or examples[summary_column][i]):
                ...
                sample = create_llama_prompt(input, is_training=True)
    ...
    def generation_preprocess_function(examples):
        ...
        for i in range(len(examples[text_column])):
            if examples[text_column][i]:
                ...
                targets.append(f"{create_llama_prompt(input, is_training=True)}")
    ...
    def postprocess_text(preds, labels):
        ...
        new_labels = []
        for pred in preds:
            splits = pred.split("[/INST]")
            if len(splits) > 1:
                new_preds.append(splits[1].strip())
            else:
                new_preds.append(pred)

        for label in labels:
            splits = label.split("[/INST]")
            if len(splits) > 1:
                new_labels.append(splits[1].strip())
            else:
