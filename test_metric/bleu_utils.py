...
def calculate_bleu1(generations, tokenizer=None, smooth=False):
    tokenizer = Tokenizer13a() if tokenizer is None else tokenizer
    bleu1 = Bleu()
    bleu1_args = prepare_for_bleu1(generations)
    bleu1_outputs = bleu1._compute(
        bleu1_args[0], bleu1_args[1], tokenizer=tokenizer, smooth=smooth
    )
    bleu1_outputs["bleuP"] = bleu1_outputs["bleu"] * 100
    return bleu1_outputs


def calculate_bleu2(generations, tokenizer=None, smooth=False):
